...
update_cpld= driver.find_element_by_css_selector('div.col-md-3:nth-child(13) > a:nth-child(1) > div:nth-child(1) > i:nth-child(1)')
update_cpld.click()
...
update_info_1 = driver.find_element_by_css_selector('.col-md-5').text
print (update_info_1)	
...
for line in gui.readlines():
	result=line.find("invalid_cpldimage")
	if result != -1:
		#image=line[13:37]
		image = line.split("=")[1]
...
try:
	...
	update_cpld = driver.find_element_by_id('filefirmware_image')
	#update_cpld = driver.find_element_by_css_selector('#filefirmware_image')
	update_cpld.send_keys(update_image)
...
print ("")
print ("")
wait_upload = WebDriverWait(driver,20,1).until(lambda wait_upload : driver.find_element_by_id('start'), message='wait fail')
time.sleep(1)
link = driver.find_element_by_css_selector('#start')
link.click()
'''
...
#section_options
time.sleep(2)
wait_upload = WebDriverWait(driver,70,1).until(lambda wait_upload : driver.find_element_by_id('section_options'), message='upload cpld image fail')
time.sleep(5)
#start
#start
link = driver.find_element_by_css_selector('#start')
...
print ("")
'''
##start to update cpld
WebDriverWait(driver,40,1).until(EC.alert_is_present())
...
WebDriverWait(driver,180,1).until(EC.alert_is_present())
alert_info=driver.switch_to.alert.text
print (alert_info)
time.sleep(2)
alert = driver.switch_to.alert
